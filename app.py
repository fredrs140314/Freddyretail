...
def estimate_traffic(domain):
    ...
    # MVP mock values. Replace later with Similarweb/Semrush/Ahrefs/Sistrix API.
    mock_traffic = {
        "ica.se": 12000000,
        "coop.se": 5800000,
        "willys.se": 7200000,
...
def normalize_brand_for_url(brand):
    brand = brand.lower()
...
def brand_matches_product(url, page_text, brand):
    ...
    brand_url = normalize_brand_for_url(brand)
    url_lower = url.lower()
...
def scan_product_url(url, brands):
    ...
    return {brand: brand_matches_product(url, page_text, brand) for brand in brands}
